...
def question2_4() -> str:
    ...
    """
    answer = (
        "As Q-learning is an off-policy algorithm, it learns the optimal policy using data generated by the behaviour policy making it more sensitive to changes in the value of gamma as compared to First-Visit Monte Carlo which is on-policy.\n"
        "Secondly, unlike Monte Carlo, Q-learning allows bootstrapping where the algorithm involves the maximum future reward being weighted by gamma making Q-learning more susceptible to changes in gamma.\n"
...
def question3_8() -> str:
    ...
    """
    answer = (
        "Firstly, the target network is updated every 2000 steps which results in sudden changes in the target values at these time steps. While employing this technique helps with stabilising training as action selection and target value estimation are separated, it leads to spikes in the DQN loss.\n"
        "Secondly, the random transitions we sample from the replay buffer may not be consecutive which leads to the loss varying from one batch to another, hence leading to the observed spike.\n"
        "Finally, the epsilon-greedy action selection leads to unexpected outcomes or rewards during exploration which may also cause spikes in the loss.\n"
    )  # TYPE YOUR ANSWER HERE (100 words max)
    return answer
